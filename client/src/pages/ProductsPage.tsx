...
const blank = (categoryId = ""): ProductForm => ({
  code: "",
  barcode: "",
  name: "",
  categoryId,
...
export function ProductsPage() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => void loadProducts(1), 220);
    return () => clearTimeout(timer);
  }, [search, categoryId, status, stockStatus, sort, order]);
  useEffect(() => {
  ...
  const change = (key: keyof ProductForm, value: string | number) =>
  ...
  const upload = async (file?: File) => {
    ...
    try {
      const result = await api<{ imagePath: string }>("/uploads/image", {
        method: "POST",
        body,
      });
      change("imagePath", result.imagePath);
  ...
  const remove = async (product: Product) => {
    if (!window.confirm("确定删除“" + product.name + "”吗？删除后无法恢复。"))
      return;
    try {
      const result = await api<{ message: string }>("/products/" + product.id, {
        method: "DELETE",
      });
      toast.success(result.message);
      await loadProducts();
  ...
  const bulk = async () => {
    ...
    if (
      bulkAction === "delete" &&
      !window.confirm("确定删除选中的 " + selected.length + " 件商品吗？")
    )
      return;
    ...
    try {
      const result = await api<{ message: string }>("/products/bulk", {
        method: "POST",
  ...
  const lowCount = useMemo(
    () => products.filter((item) => item.stock <= item.lowStock).length,
    [products],
  );
    ...
    <div className="space-y-5">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="page-subtitle">管理商品、价格、图片、状态与库存预警</p>
        </div>
        <button className="btn-primary" onClick={newProduct}>
          <Plus size={18} />
          新增商品
        </button>
      </div>
      <section className="card p-4">
        <div className="grid gap-3 lg:grid-cols-6">
          ...
          <select
            ...
            className="select"
          >
            <option value="retailPrice:desc">价格从高到低</option>
            <option value="retailPrice:asc">价格从低到高</option>
      ...
      {selected.length > 0 && (
        <section className="flex flex-wrap items-center gap-3 rounded-xl border border-brand-200 bg-brand-50 p-3 text-sm">
          ...
          >
            <option value="status">批量设置状态</option>
            <option value="category">批量修改分类</option>
            <option value="price">批量修改零售价</option>
          ...
          {bulkAction === "status" && (
            <select
              ...
              className="select w-auto py-1.5"
            >
              <option value="ON_SALE">上架（在售）</option>
              <option value="OFF_SALE">停售</option>
            </select>
          )}
          {bulkAction === "category" && (
          ...
          <button
            ...
            onClick={bulk}
          >
            {bulkAction === "delete" ? "删除选中商品" : "确认批量操作"}
          </button>
        </section>
      )}
      <section className="card">
        {loading ? (
          ...
          <>
            ...
            <PaginationBar
              {...pagination}
              onPage={(page) => void loadProducts(page)}
            />
            <div className="px-4 pb-3 text-xs text-slate-400">
              本页 {products.length} 件商品，其中 {lowCount} 件达到库存预警。
            </div>
          </>
        )}
      </section>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              ...
              <Field label="进货价">
                <input
                  ...
                  onChange={(e) =>
                    change("purchasePrice", Number(e.target.value))
                  }
                />
              </Field>
              <Field label="批发价">
              ...
              <Field label="零售价 *">
                <input
                  ...
                  onChange={(e) =>
                    change("retailPrice", Number(e.target.value))
                  }
                />
              </Field>
              <Field label="会员价">
                <input
                  ...
                  onChange={(e) =>
                    change("memberPrice", Number(e.target.value))
                  }
                />
              </Field>
              <Field label="当前库存">
              ...
              <Field label="库存预警值">
                <input
                  ...
                  min="0"
                  value={form.lowStock}
                  onChange={(e) => change("lowStock", Number(e.target.value))}
                />
              </Field>
              <Field label="供应商">
                <select
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {form.imagePath && (
                    <button
                      type="button"
                      title="点击放大查看图片"
                      onClick={() => setPreviewImage(form.imagePath)}
                      className="group relative h-12 w-12 overflow-hidden rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-500"
                    >
                      <img
                        src={form.imagePath}
                        alt="商品图片预览"
